...
from inventory.serializers.OrdersSerializer import AddOrderSerializer, OrderSerializer,  CustomerOrderSerializer
from inventory.serializers.OrderItemsSerializer import OrderItemsSerializer
from inventory.serializers.OrdersSerializer import OrderSerializer, MerchantOrderSerializer, AddOrderSerializer , AddGeneralOrderItemsSerializer
...
class PurchaseProductListView(
    ...
    def getOne(self, request, id):
        if id:
            try:
                query = """ CALL `datafunc_Mabrle_ERP_wUrdu`.`ORDR_List`("""+str(id)+""", 0, "", 1, 1000000); """     
    ...
    def getAll(self):
        try:
            query = """ CALL `datafunc_Mabrle_ERP_wUrdu`.`ORDR_List`(0, 0, "", 1, 1000000); """    
    ...
    def searchActive(self, params):
        ...
        q = dateConversion(q)
        try:
            query = """ CALL `datafunc_Mabrle_ERP_wUrdu`.`ORDR_List`(0, 0, '"""+q+"""', """+str(page)+""", """+str(parPage)+"""); """   
            my_data = pd.read_sql(query, connection)
    ...
    def post(self, request):
        ...
        try:
            ...
            # ADV_PAYMENT["PYMNT_AMNT"] = request.data["ADV_PAYMENT"]
            # ADV_PAYMENT["REC_ADD_BY"] = request.user.id
            # ADV_PAYMENT["PAYMNT_DT"] = request.data["ORDR_DT"] 
            # ADV_PAYMENT["ACCT_ID"] = request.data["MRCHNT_ACCT_ID"]
            # ADV_PAYMENT["IS_CASH"] = 1
            # acct = Accounts.objects.get(ACCT_TITLE ='Sale/Purchase' )
            # ADV_PAYMENT["EXPNS_TYP_ACCT"] = acct.ACCT_ID
            # acct = Accounts.objects.get(ACCT_ID=request.data["MRCHNT_ACCT_ID"])
            # ADV_PAYMENT["PYMENT_TO"] = acct.ACCT_TITLE
            # ADV_PAYMENT["NOTES"] = ADV_PAYMENT["NOTES"] = "Amount Received From -" + acct.ACCT_TITLE 
        except:
            pass
        if "QUOTE_ID" in request.data:
            try:
                ...
                return Response({'error': 'The Quotations Order does not exist.' , "error_ur" : "کوٹیشن آرڈر موجود نہیں ہے۔"}, status=400)
        ...
        if create_serializer.is_valid():
            ...
            # if new_sale.ADV_PAYMENT >0:
            #     # Post in Earning 
            #     create_serializer = AdddExpenseTransSerializer(data=ADV_PAYMENT)
            #     if create_serializer.is_valid():
            #         new_expense_transc = create_serializer.save()
            #         # post in ledger
            #         ledgerTransc = LedgerTransaction(isExpense=True)
            #         ledgerTransc.insertExpense( account_id=new_expense_transc.ACCT_ID.ACCT_ID, exp_transac=new_expense_transc)
            #         # post in cash ledger
            #         # if new_earning_transc.IS_CASH:
            #         ledgerTransc.cashOut(account_id=new_expense_transc.ACCT_ID.ACCT_ID, earn_transac=new_expense_transc)
    ...
    def put(self, request, id=None):
        try:
            order_to_update = Orders.objects.get(ORDR_ID=id, IS_SALE=0)
            order_id = id
        ...
        if create_order_serializer.is_valid():
                ...
                if order_item["IS_GEN_PROD"] ==1:
                    ...
                    if order_item_id in order_item:
                        ...
                        order_item["PRODUCT_ID"] = PRODUCT(
                            CAT_NAME=order_item["PROD_CAT"], PPRO_NAME=order_item["PROD_NAME"], PRO_USAGE=order_item["PROD_USAGE"], ID=request.user.id)
                        # Saving the ORDER_ITEM
                        order_Item_to_update = Order_items.objects.get(ORDR_ITEM_ID=order_item["ORDR_ITEM_ID"], ORDR_ID=order_item["ORDR_ID"])                        
                        order_item["REC_ADD_BY"] = order_Item_to_update.REC_ADD_BY                    
                        if Decimal(order_item["REQ_QTY"]) < order_Item_to_update.DLVRD_QTY:
                            return Response({'error': 'Delievered Quantity cannot be less than the actual value' ,  'error_ur': 'ڈیلیور شدہ مقدار اصل قیمت سے کم نہیں ہو سکتی' }, status=400)
            ...
            # return Response(read_serializer.data, status=201)
            return Response({'success' : "Order is updated successfully" ,  'success_ur' : "آرڈر کو کامیابی کے ساتھ اپ ڈیٹ کر دیا گیا ہے۔" } , status=status.HTTP_201_CREATED)
        return Response(create_order_serializer.errors, status=400)

    def delete(self, request, id=None):
        try:
            order_to_delete = Orders.objects.get(ORDR_ID=id, IS_SALE=0)
        except Orders.DoesNotExist:
            return Response({'error': 'The Sale Order does not exist.',  "error_ur" : "سیل آرڈر موجود نہیں ہے۔"}, status=400)
...
class MerchantPurchaseProductListView(
    APIView,  # Basic View class provided by the Django Rest Framework
    UpdateModelMixin,  # Mixin that allows the basic APIView to handle PUT HTTP requests
    # Mixin that allows the basic APIView to handle DELETE HTTP request):
    DestroyModelMixin,
        ...
        if id:
            try:
                queryset = Orders.objects.filter(
                    MRCHNT_ACCT_ID=id, IS_ACTIVE=1, IS_SALE=0)
            except Orders.DoesNotExist:
                return Response({'error': 'The Sale Order does not exist.' ,  "error_ur" : "سیل آرڈر موجود نہیں ہے۔"}, status=400)
        read_serializer = CustomerOrderSerializer(queryset, many=True)
