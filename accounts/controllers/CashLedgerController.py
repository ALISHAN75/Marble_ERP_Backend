...
class CashLedgerListView(
    ...
    def getOne(self, request, id):
        if id:
            try:
                query = """ CALL `datafunc_Mabrle_ERP_wUrdu`.`ACCT_CashLedger`(1 , """+str(id)+""", 0 , 1, 1); """     
    ...
    def getAll(self):
        try:
            query = """ CALL `datafunc_Mabrle_ERP_wUrdu`.`ACCT_CashLedger`(2, 0,0,1,1000000); """    
            my_data = pd.read_sql(query, connection)
        except ConnectionError:
            return Response({"error" : "Database Connection Error" , "error_ur" :  "ڈیٹا بیس کنکشن کی خرابی " }  , status=status.HTTP_400_BAD_REQUEST)
    ...
    def searchActive(self, params):
        ...
        if len(q)>0:
            ...
            is_id_srch = 2 
        try:
            query = """ CALL `datafunc_Mabrle_ERP_wUrdu`.`ACCT_CashLedger`("""+str(is_id_srch)+""" , '"""+q+"""', """+str(ACCT_ID)+""" ,"""+str(page)+""" , """+str(parPage)+"""); """       
            my_data = pd.read_sql(query, connection)
...
class CashLedgerByAcct(APIView):
    renderer_classes = [UserRenderer]

    def post(self, request , format=None):
        try:
            query = """ CALL `datafunc_Mabrle_ERP_wUrdu`.`ACCT_CashLedger`(1 , """+str(request.data["ACC_ID"])+""", 0 , 1, 1); """     
            my_data = pd.read_sql(query, connection)
        except ConnectionError:
            return Response({"error" : "Database Connection Error" , "error_ur" :  "ڈیٹا بیس کنکشن کی خرابی " }    , status=status.HTTP_400_BAD_REQUEST)
