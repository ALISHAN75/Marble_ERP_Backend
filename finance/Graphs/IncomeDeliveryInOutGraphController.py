...
class IncomeDeliveryInOutGraphListView(
    ...
    def searchActive(self, params):
        try:
            query = """ CALL `datafunc_Mabrle_ERP_wUrdu`.`Income_delivery_in_out_graph`(1); """       
            earning = pd.read_sql(query, connection)
            query = """ CALL `datafunc_Mabrle_ERP_wUrdu`.`Income_delivery_in_out_graph`(2); """       
        ...
        if earning.empty and expense.empty:
            ...
            graph_data = {
                "TITLE": "Delivery in and out report",     
                "TITLE_UR": "ڈیلیوری ان اور آؤٹ رپورٹ"  ,  
                "DATASET": [         
                {             
                "name": "Earning",             
